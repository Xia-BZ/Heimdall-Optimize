...
int hd_parse_command_line(int argc, char *argv[], hd_params *params) {
  ...
  while (++i < (size_t)argc) {
    ...
    else if (argv[i] == string("-pl")) {
      sscanf(argv[++i], "%d", &(params->num_execute_threads));
    }
...
void hd_print_usage() {
  ...
  cout << "    -min_tscrunch_width num  vary between high quality (large "
       ...
       << endl;
  cout << "    -t num                   set number of threads in DM trail"
       << endl;
  cout << "    -pl num                   set number of threads in pipeline"
       << endl;
}
